...
\begin{titlepage}
  ...
  {\Huge\bfseries Assignment 1 Design Report\par}
  \vspace{1cm}
  {\Large Group 5\par}
  \vspace{0.5cm}
  {\normalsize Rick de Rijk (2151940), Thom Verzantvoort (2038050), Tycho van Rooij (2080652), Jelles Duin (2146737), Gilbert Laanen (2151170)\par}
  
  \vspace{2cm}
  \includegraphics[width=0.6\textwidth]{images/Pulse Logo.png}
  
  \vfill
  \begin{flushright}
    \includegraphics[width=0.3\textwidth]{images/JADS_logo.png}
  \end{flushright}
\end{titlepage}
```
\newpage
...
![Operational Intelligence Domain](images/Operational%20Intelligence%20Domain.png){#operational-intelligence-domain width="462"}
...
**Connector Service:** The Connector Service is a microservice responsible for retrieving data from external systems via APIs on a scheduled or triggered basis. It is classified as a microservice because it encapsulates a single integration capability and operates independently per source system. An alternative would have been to implement a single unified ingestion service for all external sources, but this was rejected because it would reduce modularity, increase coupling between integrations, and make it harder to extend or maintain individual connectors.
...
**Schema Mapping Service:** The Schema Mapping Service is a data transformation service that maps source-specific data structures, such as financial records and CRM fields, into a unified internal schema. It is classified as a transformation service because it performs a dedicated and deterministic schema-conversion step within the ingestion pipeline. An alternative would have been to embed schema mapping logic directly inside each connector, but this was rejected because it would duplicate transformation logic across integrations and reduce maintainability.
...
**Data Storage Service:** The Data Storage Service is a storage component responsible for persisting both raw and normalized datasets for traceability and downstream access. It is classified as a storage service because it owns the persistence layer of the Data Integration domain. An alternative would have been to store only normalized data and discard raw ingested data, but this was rejected because retaining raw data improves traceability, debugging, and reprocessing capabilities.
...
**KPI Computation Service:** is a batch transformation service responsible for calculating business metrics from normalized data inputs delivered by the Data Integration domain. It is classified as a batch transformation service because it performs a scheduled, deterministic computation step, consuming normalized records as input and producing versioned metric outputs, without owning any persistent state itself.
...
**Historical Storage Service:** is a storage service responsible for persisting and versioning computed metrics over time, forming the domain's private analytical store, the Gold Layer. It is classified as a storage service because it owns the domain's entire data state, acting as the persistence boundary that qualifies the domain as a self-contained data product. No other domain accesses this store directly.
...
**Data Serving Layer:** is a query API service that exposes pre-computed metrics from the Gold Layer to downstream consumers through a versioned, standardized interface. It is classified as a query API service because it provides a stable, explicitly defined read contract for external consumers, specifically the Operational Intelligence Orchestrator Agent, without exposing the underlying storage schema.
...
**Composition Service:** The Composition Service is a microservice responsible for assembling report sections by combining structured insights from the Operational Intelligence domain into a tenant-specific report. It is classified as a microservice because it encapsulates a single business capability and operates independently within the reporting pipeline. An alternative would have been to let the Operational Intelligence domain generate fully composed reports, but this was rejected because it would mix reasoning and presentation concerns, violating separation of concerns.
...
**Template Service:** The Template Service is a template management component that applies predefined templates to structure insights into consistent layouts and sections for each tenant. It is classified as a template service because it separates presentation structure from report content. An alternative would have been to hardcode report structures within the Composition Service, but this was rejected because it would reduce flexibility and limit reuse across tenants.
...
**Scheduling Service:** The Scheduling Service is a scheduling component responsible for triggering report generation and delivery based on configured intervals. It is classified as a scheduling service because it manages time-based orchestration independently of report generation logic. An alternative would have been to trigger reporting exclusively through upstream events, but this was rejected because reporting requires predictable, time-based delivery independent of pipeline execution timing.
...
**Delivery Service:** The Delivery Service is a microservice responsible for distributing generated reports to stakeholders via configured channels such as email and PDF attachments. It is classified as a microservice because it encapsulates delivery logic and can scale independently of report generation. An alternative would have been to embed delivery logic within the Composition Service, but this was rejected because it would tightly couple report generation and distribution, reducing flexibility and scalability.
...
The Tenant Management domain is a generic domain responsible for onboarding SME clients, managing tenant-specific configuration, and controlling user access within the Pulse platform. It owns tenant registration, user accounts, role assignments, and connector configuration metadata. It does not ingest business data, compute KPIs, generate insights, or deliver reports. Its responsibility is purely administrative and configurational.
...
**Tenant Onboarding Service**: is a microservice that creates new tenants and initializes their baseline workspace configuration.
...
**User and Role Management Service**: is a microservice that manages user accounts and role assignments within each tenant.
...
**Tenant Configuration Service**: is a configuration service that stores and exposes tenant-level settings such as connector metadata and report preferences.
...
**Access Control Service**: is a validation service that enforces tenant boundaries and authorizes access to tenant resources.
...
The Notification domain is a generic domain responsible for dispatching event-driven alerts to stakeholders via email, SMS, and Slack. It owns routing logic, per-channel message formatting, and reliable delivery. It is triggered by events from other domains such as report delivery confirmations from Reporting & Delivery and connector failure events from Data Integration. It does not generate insights, compose reports, or determine the business significance of events.
...
**Notification Router Service** is a microservice that receives incoming events and determines the target channel based on event type and recipient preferences.
...
**Message Formatting Service** is a microservice that transforms notification content into the appropriate channel format.
...
**Dispatch Service** is a microservice that sends formatted messages to external providers and records delivery outcomes.
...
![Design-Architecture](images/ADA-Design-Architecture.png){#fig-design-architecture-section3 width="547"}
...
The alternative of chaining REST calls between domains was rejected precisely because it would have introduced tight temporal coupling, where the failure or slowness of any single domain would propagate upstream and stall the entire pipeline. 
...
During the preparation of this work, we used Claude (Sonnet 4.6), ChatGPT (GPT-5.3), and Perplexity to support the design process. Claude and ChatGPT were used primarily for brainstorming, design sparring, and interactive guidance, prompting us to reflect on design decisions and develop our own solutions rather than generating content directly, as well as grammar and clarity review. Perplexity was used for research and source discovery. The following parts of the assignment were affected: Section 1 (brainstorming and grammar review), Section 2 (design guidance and grammar review), Section 3 (grammar review). After using these tools, Rick de Rijk, Thom Verzantvoort, Tycho van Rooij, Jelles Duin, and Gilbert Laanen evaluated the validity of all outputs and edited the content as needed. As a consequence, Rick de Rijk, Thom Verzantvoort, Tycho van Rooij, Jelles Duin, and Gilbert Laanen take full responsibility for the content of this work.
...
![Data Integration Domain](images/Context%20Diagram%20Data%20integration%20service.png){#fig-data-integration width="446"}

![KPI & Analysis Domain](images/KPI-Domain-design.png){#fig-kpi-domain}

![Reporting & Delivery Domain](images/Domain-Report.png){#fig-diagram-report}
